...
import { GetAllBannerListForAdmin, DeleteBannersForAdmin } from '@/store/slices/adminAction';
...
const BannieresList = () => {
    ...
    const { Loading, success, errors, AdminBannersList } = useSelector((state: RootState) => state.lindicateur);
    ...
    const handleSearch = (value: any) => {
        dispatch(GetAllBannerListForAdmin({ token, page: 1, search: value }));
    }
    ...
    useEffect(() => {
        if (token) {
            dispatch(GetAllBannerListForAdmin({ token, page: 1 }));
        }
    }, [dispatch, token])
    ...
    const deleteDetails = (id: any) => {
        Swal.fire({
            ...
            if (result?.isConfirmed) {
                dispatch(DeleteBannersForAdmin({ token, id }));
            }
        })
    }
    ...
    const handlePageClick = (event: any) => {
        ...
        dispatch(GetAllBannerListForAdmin({ token, page: newOffset }));
    }
            ...
            <div className="w-full lg:w-auto">
                ...
                <div className="flex flex-col lg:flex-row gap-5 w-full lg:justify-between py-4">
                    ...
                    <div>
                        ...
                        >
                            <a href="/admin/ajouter-une-banniere/">
                ...
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg pt-4 w-full pb-20">
                    {
                        AdminBannersList?.data?.data?.length > 0 ?
                            <>
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 border-2">
                                    ...
                                    <tbody>
                                        {
                                            AdminBannersList?.data?.data?.map((data: any, i: number) => {
                                                return (
                                                    <>
